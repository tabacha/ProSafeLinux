...
def pack_mac(value):
    if (len(value) == 17):
        return binascii.unhexlify(value[0:2]+value[3:5]+value[6:8]+value[9:11]+
                                  value[12:14]+value[15:17])
    if (len(value) == 12):
...
def unpack_mac(value):
    ...
    return (mac[0:2]+":"+mac[2:4]+":"+mac[4:6]+":"+mac[6:8]+":"+mac[8:10]+":"+
           mac[10:12])
...
class ProSafeLinux:
    ...
    CMD_BROADCAST_FILTER =  psl_typ.PslTypBroadcastFilter(0x5800, "broadcast-filter")
    ...
    CMD_IGMP_HEADER_VALIDATION =  psl_typ.PslTypBoolean(0x7000, "igmp-header-validation")
